...
def train_fedprox(
    ...
    global_model_params = copy.deepcopy(list(net.parameters()))
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in enumerate(trainloader):
            ...
            proximal_term = 0.0
            for local_param, global_param in zip(net.parameters(), global_model_params):
                proximal_term += torch.norm(local_param - global_param.to(device)) ** 2
            
            proximal_loss = (proximal_mu / 2) * proximal_term
            ...
            epoch_proximal_loss += proximal_loss.item() * target.size(0)
            _, predicted = torch.max(output.data, 1)
            train_total += target.size(0)
            train_correct += (predicted == target).sum().item()
        
        avg_train_loss = epoch_train_loss / train_total if train_total > 0 else 0
        avg_proximal_loss = epoch_proximal_loss / train_total if train_total > 0 else 0
        train_acc = train_correct / train_total if train_total > 0 else 0
        
        history["train_loss"].append(avg_train_loss)
        history["train_acc"].append(train_acc)
        history["proximal_loss"].append(avg_proximal_loss)
        total_proximal_loss += avg_proximal_loss
    ...
    metrics = {
        ...
        "proximal_loss": total_proximal_loss / epochs if epochs > 0 else 0,
