...
from server_pars import ParsServer, gen_evaluate_fn
from strategy import ParsStrategy
        ...
        if isinstance(saved_parameters, list):
            ...
            if "parameters" in saved_parameters:
                ...
                if isinstance(actual_parameters, list):
                    ...
                    print(
                        f"Warning: Unsupported wrapped parameter format in {weight_file}. Starting with random weights.")
                    return None
            else:
                state_dict = {}
...
class BestModelTracker:
    ...
    def track_round_metrics(self, round_num: int, metrics: Dict[str, float]) -> None:
...
@hydra.main(config_path="conf", config_name="pars", version_base=None)
def main(cfg: DictConfig) -> None:
    ...
    client_fn = call(
        ...
        valloaders,
        model=cfg.model,
        client_cv_dir=os.path.join(HydraConfig.get().runtime.output_dir, "client_cvs"),
    )
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print(f"PARS EXPERIMENT SUMMARY")
        print("=" * 60)
        ...
        try:
            ...
            if best_model_metrics:
                print("\n" + "=" * 60)
                print("PARS EXPERIMENT SUMMARY (After Error)")
                print("=" * 60)
                print("\nBest Model Metrics:")
                print(f"  Round: {best_model_metrics.get('best_round', 'N/A')}")
                print(f"  Convergence Round: {best_model_metrics.get('convergence_round', 'N/A')}")
