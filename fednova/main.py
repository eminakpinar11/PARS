...
def customize_server(server: Server, model_tracker) -> Server:
    ...
    def enhanced_evaluate_round(server_round: int, timeout: Optional[float] = None) -> Optional[Tuple[Optional[float], Dict[str, Scalar]]]:
        ...
        if result is not None:
            ...
            model_tracker.update(server_round, parameters_ndarrays, accuracy)
            model_tracker.track_round_metrics(server_round, metrics)
            
            strategy_name = server.strategy.__class__.__name__
            print(f"Round {server_round}: Server evaluation completed ({strategy_name}, Accuracy: {accuracy:.4f})")
...
def main(cfg: DictConfig) -> None:
    ...
    try:
        ...
        print("\n" + "="*60)
        print("EXPERIMENT SUMMARY (FedNova)")
        print("="*60)
        ...
        print("\nError occurred, but trying to evaluate best model...")
        ...
        try:
            ...
            print(f"A second error occurred while evaluating best model: {e2}")
