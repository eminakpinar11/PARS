...
from models_adaptive import test, test_with_metrics, train_adaptive


class FlowerClientAdaptive(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config: Dict[str, Scalar]):
        ...
        self.net, training_metrics = train_adaptive(
            self.net,
            self.trainloader,
            validation_loader,
            self.device,
        ...
        if self.validation_enabled and len(self.valloader.dataset) > 0:
            ...
            print(f"Client {self.cid} Round {self.round} Validation Results ({self.strategy_name}):")
            ...
            print(f"Client {self.cid} Round {self.round}: Validation disabled ({self.strategy_name})")
        ...
        metrics = {
            ...
            "training_time": float(training_metrics["training_time"]),
    ...
    def evaluate(self, parameters, config: Dict[str, Scalar]):
        ...
        try:
            ...
            print(f"Client {self.cid} Server-side Evaluation ({self.strategy_name}):")
...
def gen_client_fn(
    ...
    strategy_name: str = "AdaptiveFL",
) -> Callable[[str], FlowerClientAdaptive]:

    def client_fn(cid: str) -> FlowerClientAdaptive:
        ...
        if not validation_enabled:
            print(f"Client {cid}: Validation disabled (empty validation dataset) - {strategy_name}")
        else:
            print(f"Client {cid}: Validation enabled ({len(valloader.dataset)} validation samples) - {strategy_name}")

        client = FlowerClientAdaptive(
            net,
            trainloader,
            valloader,
            device,
