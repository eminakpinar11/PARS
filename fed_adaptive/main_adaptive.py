...
from models_adaptive import test_with_metrics
...
def get_strategy_info(cfg: DictConfig) -> Dict[str, Any]:
    strategy_config = cfg.strategy
    strategy_name = strategy_config._target_.split('.')[-1]
    
    strategy_info = {
        "strategy_name": strategy_name,
        "strategy_target": strategy_config._target_
    }
    
    adaptive_params = {}
    for param in ["eta", "eta_l", "beta_1", "beta_2", "tau"]:
        if param in strategy_config:
            adaptive_params[param] = strategy_config[param]
    
    if adaptive_params:
        strategy_info["adaptive_params"] = adaptive_params
        print(f"\nUsing {strategy_name} with parameters:")
...
def gen_evaluate_fn(
    ...
    def evaluate(
        ...
        from models_adaptive import test
...
@hydra.main(config_path="conf", config_name="fedyogi_base", version_base=None)
def main(cfg: DictConfig) -> None:
    
    if "mnist" in cfg.dataset_name:
        cfg.model.input_dim = 256
        cfg.model._target_ = "models_adaptive.CNNMnist"
    ...
    strategy_info = get_strategy_info(cfg)
    ...
    if initial_model is not None:
        ...
        print("Creating random initial parameters for adaptive strategy...")
        net = instantiate(cfg.model).to(device)
        initial_parameters = [val.cpu().numpy() for _, val in net.state_dict().items()]
        print(f"✓ Random initial weights created - {len(initial_parameters)} parameter groups")
        
        evaluate_fn = gen_evaluate_fn(testloader, device=device, model=cfg.model)
        
        initial_loss, initial_metrics = evaluate_fn(0, initial_parameters, {})
        print(f"Initial model accuracy (with random weights): {initial_metrics.get('accuracy', 0.0):.4f}")
    ...
    print(f"✓ Strategy configured with initial parameters ({strategy_name})")
    

    strategy = instantiate(cfg.strategy, **strategy_kwargs)
    
    def enhanced_evaluate_round(*args, **kwargs):
        
        orig_evaluate_round = strategy.__class__.evaluate
        
        result = orig_evaluate_round(strategy, *args, **kwargs)
        ...
        if result is not None:
            ...
            evaluation_type = metrics.get("evaluation_type", "federated")
            server_round = args[0] if args else kwargs.get('server_round', 0)
            
            if evaluation_type == "centralized":
                print(f"Round {server_round}: Centralized evaluation completed (Accuracy: {accuracy:.4f})")
                model_tracker.track_round_metrics(server_round, metrics, "centralized")
            else:
                print(f"Round {server_round}: Federated evaluation completed (Accuracy: {accuracy:.4f})")
                model_tracker.track_round_metrics(server_round, metrics, "federated")
        
        return result
    
    strategy.evaluate = enhanced_evaluate_round
    ...
    try:
        ...
        print("\n" + "="*60)
        print(f"EXPERIMENT SUMMARY ({strategy_name})")
        print("="*60)
        print(f"Strategy: {strategy_name}")
        if "adaptive_params" in strategy_info:
            print("Strategy Parameters:")
            for param, value in strategy_info["adaptive_params"].items():
                print(f"  {param}: {value}")
        ...
        try:
            ...
            if best_model_metrics:
                print("\n" + "="*60)
                print(f"EXPERIMENT SUMMARY (After Error) - {strategy_name}")
                print("="*60)
                print("\nBest Model Metrics:")
                print(f"  Round: {best_model_metrics.get('best_round', 'N/A')}")
                print(f"  Convergence Round: {best_model_metrics.get('convergence_round', 'N/A')}")
