...
def gen_evaluate_fn(
    ...
    def evaluate(
        ...
        try:
            ...
            num_model_params = len(list(net.parameters()))
            total_params = len(parameters_ndarrays)
            
            print(f"Central evaluation: Model expects {num_model_params} params, received {total_params}")
            
            if total_params == num_model_params * 2:
                model_parameters = parameters_ndarrays[:num_model_params]
                print(f"SCAFFOLD format detected: Using first {num_model_params} parameters for central evaluation")
            elif total_params == num_model_params:
                model_parameters = parameters_ndarrays
                print(f"Standard format detected: Using all {num_model_params} parameters for central evaluation")
            else:
                print(f"Warning: Parameter count mismatch. Expected {num_model_params} or {num_model_params*2}, got {total_params}")
                model_parameters = parameters_ndarrays[:num_model_params]
            
            params_dict = zip(net.state_dict().keys(), model_parameters)
            ...
            metrics = {
                "accuracy": accuracy,
                "evaluation_type": "centralized"
            }
            
            return loss, metrics
            
        except Exception as e:
            print(f"Central evaluation error: {e}")
            import traceback
            traceback.print_exc()
            return 0.0, {"accuracy": 0.0, "evaluation_type": "centralized", "error": str(e)}
